...
class LocalFlightsPage extends StatefulWidget {
  final User user;

  const LocalFlightsPage({
    super.key,
    required this.user,
  });

  @override
  State<LocalFlightsPage> createState() => _LocalFlightsPageState();
}

class _LocalFlightsPageState extends State<LocalFlightsPage> {
  ...
  Future<void> _loadFlights() async {
    ...
    try {
      final flights = await _databaseHelper.getFlights('local');
      if (mounted) {
        setState(() {
          _flights = flights
  ...
  Future<void> _addFlight() async {
    ...
    final Flight? newFlight = await showDialog<Flight>(
      context: context,
      builder: (context) => const AddFlightDialog(flightType: 'local'),
    );
